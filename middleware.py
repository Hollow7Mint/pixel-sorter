"""Pixel Sorter — middleware for effect payloads."""
...
class PixelMiddleware:
    """Middleware for Pixel Sorter effect payloads."""
...
def parse_effects(payload: str) -> List[Dict[str, Any]]:
    ...
    return [PixelMiddleware._coerce(item) for item in raw]


def render_effect_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Effect to a JSON string."""
    if indent is None:
        return PixelMiddleware.dumps(record)
